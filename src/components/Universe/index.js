  ...
  render() {
    ...
    var filteredProjects = projects.filter(
      (project) =>
        ...
        (filter.length === 0 ||
          filter.filter((f) => Object.keys(project.Categories).includes(f))
            .length > 0) &&
        (project.Title.toLowerCase().includes(textFilter.toLowerCase()) ||
          project.Author.DisplayName.toLowerCase().includes(
            textFilter.toLowerCase()
          )) &&
        (!classFilter || classMembers.includes(project.Author.key))
    );

    const selectedFilters = Object.keys(CATEGORIES).filter((v) =>
      filter.includes(v)
    );
